...
| `targetLevel` | Big Win Popup 最终应到达的等级；等级数量、顺序和 key 必须来自当前资源工程，不能默认固定为三级 | 数字跨阈值但标题/VFX 没升级，或把其他项目的等级枚举直接搬入 |
...
| `PLAYING` | 激活全屏 root/遮罩，播放资源工程定义的起始级别，数字从 0 滚到目标，跨阈值时原子切换整套等级表现 | Skip 延迟结束后可点击 | 自然滚到目标，或第一次 Skip 直接补齐最终金额/目标等级 |
...
Big Win 的运行时表现以当前游戏的资源工程为准，不以目标工程现有实现、其他游戏、截图或通用命名为准。分析与实现前必须在资源工程中实际运行一次自然滚分、跨级、Skip 和退场，并按以下优先级取证：

1. 资源工程运行时的节点 `active`、SpriteFrame、动画状态、粒子、音频和加载日志。
2. 资源工程控制器的切级顺序、动态加载路径、动画事件和清理逻辑。
3. Prefab/Scene 的序列化引用、节点层级、UUID/meta、默认激活状态和动画 clip 绑定。
4. 静态图片或竞品画面只能补充视觉结果，不能证明资源路径、回调或切级时序。

为当前项目建立一份等级资源契约；没有证据的字段标记 `UNKNOWN`，不得从 YJZR、JDSRY 或名称相似的资源猜测：

| 字段 | 必须记录的内容 |
| --- | --- |
| 等级 | 资源工程真实 `levelKey`、显示名称（大奖/巨奖/超级大奖或项目自定义）、顺序和最终等级 |
...
如果资源工程无法启动、目标等级资源不完整、Prefab 绑定与运行时行为矛盾且无法解释，状态为 `NEEDS_RESOURCE_BASELINE`。不得用普通 Label、相近 Sprite、其他游戏 VFX 或猜测路径补齐后宣称还原完成。

#### 3.6.4 Sprite、动画、音效与动态资源就绪门禁

先识别资源工程原本采用的引用方式，再保持同一种运行时契约：

| 资源来源 | 接入规则 | 禁止行为 |
| --- | --- | --- |
| Prefab 序列化 `@property` | 保留组件类型、SpriteFrame/AnimationClip/AudioClip 引用、UUID 和节点绑定；实例化后直接使用 | 为了“统一”擅自改成字符串路径动态加载 |
| `SpriteAtlas` | 加载资源工程实际 atlas，并用已验证的精确 frame key 取 SpriteFrame；同时核对 plist/texture/meta | 猜文件名、扩展名、子资源后缀或大小写 |
| `resources`/Bundle | 使用资源工程运行时代码证明的 bundle、相对路径和资产类型；记录 bundle 就绪时点 | 把 `assets/resources` 文件系统路径直接当运行时地址，或混用其他游戏的路径常量 |
| 动态 `AudioClip`/音频服务 | 保持资源工程使用的 AudioClip 类型或音频服务 key；验证 bundle/path、缓存、bus/channel、loop 和返回 handle | 猜音效 key、绕过既有音频服务、按文件名推断路径，或用全局 `stopAll` 清理单个 Popup |
...
| 本地化标题 | 按资源工程的语言 key、atlas 或 bundle 切换；加载请求绑定当前语言与表现代号 | 旧语言异步结果返回后覆盖当前语言标题 |

资源门禁必须满足：

- 按资源工程策略在打开 Popup 前预载全部等级，或至少预载当前级和下一可能等级。没有运行证据时优先完整预载，避免滚分已经跨级才开始找资源。
...
- 分析阶段发现资源缺失必须阻断验收。已发布运行时仍要保证结算 callback 不被卡死；其降级方式优先复刻资源工程。资源工程没有可证明的降级时，只能保底显示最终金额并记录视觉/音频未还原，不能以保底表现通过验收。
...
显示金额跨过 nextLevel.levelStartPayout
  ...
  -> 停止当前级独有的 transient/loop，保留资源工程定义的共用滚分与金币效果
...
- Tween 单帧跨过多个阈值时，是否依次播放 BIG -> MEGA -> SUPER MEGA，或直接到目标级，必须照资源工程运行时行为。需要逐级时使用队列，前一级切换完成后才进入下一级；不得同帧启动多套动画和音效。
...
- 共用滚分 BGM/金币雨是否跨级持续、每级 sting 是否叠加、旧级 loop 是否淡出，都以资源工程为准。先检查 Animation Event，避免代码和 clip 同时播放同一音效或生成同一 VFX。
...
锁定输入并使旧 tween/load/animation callback 失效
  ...
  -> 按资源工程规则直接套用最终级，或完成必须保留的中间切级
  -> 最终级高潮表现只触发一次
  -> 进入 WAITING，不直接重复 Dismiss/callback
```

如果 Skip 时最终级资源仍未 ready，不得先显示混搭资源。等待、取消或降级路径必须跟随资源工程；无可证明路径时使用最终金额保底并记录视觉失败，但结算流程仍必须可结束。

不同表现组件的切换与清理重点：

| 组件 | 切级动作 | Reset/Dismiss |
| --- | --- | --- |
| Sprite/BitmapFont | 新资源完整赋值后再显示；保持资源工程的 SizeMode、颜色、材质和本地化选择 | 清除仅本次持有的动态引用，恢复初始 holder 与语言状态 |
| Animation | 停止旧级 clip，复位新级播放位置，先注册一次性完成监听再播放；事件帧不得重复派发 | 使用相同 callback/target 解绑，停止并复位所有等级 clip |
| Spine | 按真实 track/animation/mix 播放，切级前清理旧 track 或监听 | 清除 complete listener/track，并恢复资源工程默认皮肤与动画 |
| Particle/VFX | 每级爆发只 `reset/play` 一次；共用 loop 按资源工程决定是否持续 | 停止发射、清理残粒子并恢复节点 active |
...
每次 `show` 创建新的 `audioSessionId`，并记录进入 Popup 前的模式与资源工程实际音频状态。所有异步加载、Animation Event、延迟播放、fade 完成和音频完成回调都要同时验证 `audioSessionId + presentationGeneration + state`；只检查节点是否存在不足以阻止旧会话串音。

| 音频角色 | 启动与切级 | Skip/WAITING | Dismiss/异常 |
| --- | --- | --- | --- |
| 场景 BGM | 按资源工程选择 pause、stop 或 fade；保存 Normal/FG 模式及恢复责任，不能只看当前节点名猜测 | 通常不在首次 Skip 时提前恢复 | 只由当前会话恢复一次正确的 Normal/FG BGM；新模式已接管时不得用旧回调覆盖 |
| 滚分 loop | Popup 入场后启动一次并保存 handle；跨级是否持续、升调或换 clip 以资源工程为准 | 到额时停止/淡出或切到 final 音，只执行一次 | 按 handle 停止当前会话拥有的 loop，禁止全局停止其他 UI/场景音效 |
| 等级 sting/voice | 成功进入每一级时最多一次；播放顺序、是否允许前后级重叠、是否先停旧 voice 以资源工程为准 | 直达最终级时取消未进入等级的排队音；是否补播中间级只按资源工程行为 | 取消未触发的延迟任务，停止资源工程要求中断的 voice/sting |
| 到额/final 音 | 自然滚到 `targetPayout` 时由唯一入口触发 | Skip 到额与自然完成竞争时仍只播放一次 | 不作为 callback 是否完成的隐式替代，除非资源工程明确由音频完成事件推进状态 |
| 退场音 | 只在进入 `DISMISSING` 时播放一次 | 第二次 Skip 和自动退场竞争时由状态 guard 去重 | 即使 clip 缺失也必须完成视觉退场和结算 callback |

音频实现必须满足：

- 每个音效在等级资源契约中指定唯一触发者：控制器代码或 Animation Event 二选一。若资源工程两处都存在，必须通过运行日志确认其中一处是 guard/fallback，而不是在目标工程照搬成双播。
- 保存 Popup 自己启动的 loop、voice、sting 和 fade handle，并按 handle/所属 bus 停止。不得使用会影响 Reel、按钮、InfoBoard 或场景 BGM 的宽泛全局停止。
- 动态 AudioClip 在相应等级整体 ready 前不得参与切级；加载失败不能让金额 tween、WAITING、Dismiss 或 callback 永久等待。视觉和音频降级都必须记录，且不能冒充与资源工程一致。
- 快速跨级时，音效按资源工程选择逐级队列或直达最终级；队列中的每项携带目标 level 和 generation。`displayLevel` 已越过、发生 Skip 或会话结束后，旧项必须丢弃。
- 首次 Skip 先使旧金额 tween、等级队列、延迟音效和动画事件失效，再应用最终级。最终级 sting/voice/final 音各自是否播放，必须与资源工程同一 Skip 时点一致，不能为了“有声音”全部补播。
...
| BW-15 | 自然滚分依次跨过 Big、Mega、Super Mega | 按资源工程顺序切级；每级 holder、标题、VFX 和 sting 原子切换且只进入一次，无混搭或空白帧 |
| BW-16 | 单帧金额更新跨过两个以上阈值 | 逐级队列或直达逻辑与资源工程一致，不同级动画/音效不并发抢状态 |
...
| BW-19 | 同一 Sprite/atlas 被连续金额帧请求 | 实际只发起一次加载并复用缓存；frame key、资源类型、路径与资源工程日志一致 |
| BW-20 | BigWin 播放中切换语言 | 标题按资源工程刷新；旧语言请求返回后不得覆盖当前语言，不影响金额和等级状态 |
| BW-21 | 前一轮达到最高级，关闭后下一轮只达到 Big | 所有 holder、clip、Spine track、粒子、Sprite、音频和 `displayLevel` 已复位，不残留最高级表现 |
| BW-22 | 人为缺失目标级 Sprite/atlas/clip/AudioClip | 分析验收标记 `NEEDS_RESOURCE_BASELINE`；运行时不阻塞结算但明确记录视觉/音频失败，不用近似资源冒充通过 |
| BW-23 | 同时检查资源工程与目标工程运行时 | 节点 active、SpriteFrame/atlas key、clip、粒子、音频、切级顺序和动态加载日志逐项一致；差异有证据和结论 |
| BW-24 | 自然依次进入 Big、Mega、Super Mega | 每级 sting/voice 按资源工程顺序各播放一次；滚分 loop 的持续/换音、旧 voice 的停止/重叠均一致 |
| BW-25 | 同一等级的 Animation Event 与代码路径同时可达 | 只有等级资源契约指定的触发者实际播放，日志和音频 handle 证明没有双播 |
| BW-26 | Big 阶段第一次 Skip 直达 Super Mega | 未进入等级的排队音被取消；最终级 sting/voice/final 音的播放组合和时点与资源工程一致，各自最多一次 |
...
| BW-30 | 动态 AudioClip 连续请求、加载失败后再次打开 Popup | 同一路径请求去重；失败不阻塞结算；新会话不复用失效 handle，缓存与 release 次数符合资源工程拥有关系 |
...
- 保存资源工程运行时与目标工程运行时的切级时间线：金额、`displayLevel`、`targetLevel`、`presentationGeneration`、加载开始/完成、holder active、clip/VFX/SFX 和 callback。
