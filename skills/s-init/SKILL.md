...
- `s_cli` in `s_init planning contribution` mode returns client modules, candidate owners, dependencies, verification, acceptance, and client-side gaps. It remains read-only, creates no `doc/s_cli/**`, and does not browse the URL itself.
...
- `s_cli` in `s_init detailed planning contribution` mode returns the client implementation breakdown, evidence-to-owner mapping, state/timing and cleanup rules, server-fixture dependencies, Cocos/static/runtime/visual acceptance, and client Gates.
