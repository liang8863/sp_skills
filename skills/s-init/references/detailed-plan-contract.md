...
4. Ordered implementation slices with exact candidate areas, prerequisites, server fixture dependencies, state/timing requirements, cleanup/restore behavior, and write-scope risks. When `{replicationId}_res/loading` exists, the client plan must include the LoadingPanel resource-to-`Sprite.spriteFrame` binding slice and its missing/unsafe-source Gate. Exact `affectedFiles` remain the responsibility of each formal `s_cli` version 2 plan.
...
For the Drop peeking and Spin peeking rows, acceptance must additionally identify the target Reel's real visible bounds and mask/clipping geometry (columns, rows, spacing, anchor, scale, and coordinate space), then verify that the replacement mask matches those bounds. Record each column effect's owner, intended column coverage, and sibling/layer/z-order; reject missing-column, cross-column, out-of-bounds, or unintended Reel/Symbol/UI occlusion. These geometry and coverage claims require target-local Prefab/Scene, archived JS, or runtime evidence and must be rechecked in every applicable Drop/Spin peeking, Turbo, or Stop fixture.
