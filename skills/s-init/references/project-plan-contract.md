...
When `<project>/assets/resources/{replicationId}_res/loading/` contains a safe image or `cc.SpriteFrame`, the client workstream must include an explicit LoadingPanel binding task: exact target-local loading resource -> `Sprite.spriteFrame` on the target `LoadingPanel` loading-image node. Record the source path, AssetDB/UUID and `.meta` evidence, target node/component binding, initial active/visibility state, and separate static/runtime acceptance. A missing or unsafe source or target binding is a named `NEEDS_LOCAL_REFERENCE` or `CLIENT_CONTRACT_GAP`, not a fallback opportunity.
...
For the Drop peeking and Spin peeking rows, the plan must also map the replacement mask/clipping to the target Reel's actual visible columns and rows, including spacing, anchor, scale, and coordinate space. It must record per-column effect coverage and sibling/layer/z-order, with acceptance that rejects leaks, gaps, cross-column coverage, and unintended Reel/Symbol/UI occlusion. Only target-local Prefab/Scene, archived JS, or runtime evidence may justify a geometry or coverage exception.
