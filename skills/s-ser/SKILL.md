...
## Symbol special-attribute encoding

When a server Symbol has special attributes such as long-cell span or multiplier, encode those attributes in the server's agreed cell/symbol value so the response is self-describing; do not require the client to infer or invent them from presentation state. For games whose verified contract uses decimal packed fields, the usual low-to-high layout may reserve the units/tens for the base Symbol ID, hundreds for span/long-cell length, thousands for multiplier, and higher positions for additional proven attributes. Under that contract, `3201` means multiplier `3`, span `2`, base Symbol `01`. This is an example, not a universal protocol: derive each game's bit/decimal allocation, defaults, continuation markers, valid ranges, and collision rules from its specification, runner, mapper, and deterministic round-trip tests, and document the encode/decode authority in the response contract.

## Scheme discovery and conditional structure gate

For probability, generation, payout or field-meaning work, read [references/rtp-math-evidence.md](references/rtp-math-evidence.md). Discover the matching mathematical Office documents and applicable basic rules alongside JSON. Do not report missing semantics from JSON alone; trace the documented field through parsing to its active RNG/selection call site.
...
4. After the pull, resolve exactly one target-game JSON from the edition already selected by the user or accepted task contract. Default to `试玩版json/{GAME_CODE}_*_试玩版.json` only when no edition was selected; an already authorized Formal reference uses `正式版json/{GAME_CODE}_*_正式版.json` without asking again because Trial is absent. Never create a reference or select another game. Return `NEEDS_SCHEME_REFERENCE_DECISION` when no selected-edition match exists and `NEEDS_SCHEME_REFERENCE` when it is ambiguous or unreadable; continue scheme-independent work.
...
6. When the runner has no `scheme.json`, automatically use the uniquely matched, valid selected-edition JSON as the available scheme source. Return `VALID` with `validationMode: REFERENCE_FALLBACK`, `runnerSchemePresent: false`, and `structureCompared: false`; do not create or copy a runner-local file. The missing local file is not a task Gate.
7. Run `powershell -NoProfile -ExecutionPolicy Bypass -File "<slot-fe-client-root>/.codex/skills/s-ser/scripts/validate-scheme-structure.ps1" -RunnerPath "<runner>" [-GameCode "<GAME_CODE>"] [-ReferenceEdition Trial|Formal]` and save its JSON result in the normal evidence root. A `demo.json`, workflow file, successful build, or successful startup does not replace scheme discovery when the current work depends on scheme data.
