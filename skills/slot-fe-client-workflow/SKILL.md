...
- For non-`@s_cli` YJZR visual/audio/UI work, first use assets under `games/yjzr/assets/resources/yjzr_res`. Fall back to the shared/resource project only when the needed asset is absent there. For duplicate localized atlas frame names, prefer UUIDs from `resources/yjzr_res/**.meta`.
- `@s_cli` does not load this workflow skill. If it is encountered indirectly, its stricter local-reference contract overrides every resource-project default in this index: use only the target project's `assets/resources/{gameId}_res/`, `doc/js_scripts/`, and `doc/project_info.md`, with no external-project fallback. During `@s_cli`, do not load or read `.codex/references/workflow-concepts.md`, do not resolve or use the default resource project, and do not read or use `.codex/agent-team.yaml` `resource_materials` or `workflow_defaults`.
- For workflows other than `@s_cli`, read `.codex/references/workflow-concepts.md` for common directory definitions, resource project path, docs output rules, Cocos MCP port convention, and Git routing constraints.
- For workflows other than `@s_cli`, common directories are: `.codex` stores Codex skills/agents/config; `docs` stores output documents grouped by coarse folder; `E:\CCCCCC\UIProj\387Proj\NewProject` is the default resource project; `E:\CCCCCC\slot-fe-client` is the project root; `games` contains game instances, with `yjzr` currently in active development.
- For PM short workflows other than `@s_cli`, read `.codex/agent-team.yaml.workflow_defaults`; missing project, allowed agents, reference materials, expected delivery, and stop conditions should be filled from that config unless the current user message overrides them. Its `resource_materials` and `workflow_defaults` are not inputs to `@s_cli`.
